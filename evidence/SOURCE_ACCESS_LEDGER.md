...
| 20 | **EcoSoft OST broker web terminal** (`ost.ecosoftbd.com`, the account holder's own broker login) | HAR of the owner's own session, recorded by the owner 2026-09-07 14:22–14:31 UTC (market closed); processed by `seeing/capture/har_probe.py` (response bodies only, sensitive keys stripped, 0 leaked) | **V** — 58 market records: `GET /core/api/v1/Order/MarketDepth` ×49 (11 s polls, JSON, full 10-row snapshot each time, no WebSocket) + `GET /core/api/v1/Analysis/IndexData` ×9 (60 s). Adapter `ecosoft_ost.py`; map `evidence/ecosoft_ost/SCHEMA_MAP.md` | top-10 bid/ask price+qty, LTP/O/H/L/close/YCP, day trades/volume/value, `DateTime` stamp (tz convention unresolved on a closed-market recording), DSEX index + breadth + market totals | **order counts, prints, side, queue, sequence id** — same ceiling as #1/#4; static book (48/48 identical) because the market was closed → an open-market recording is the next step. Second recording 15:41 UTC (AAMRANET, 1-level book) cross-checked against #4 at the same minute: identical → the terminal republishes the exchange book; `Depth.DateTime` ≠ exchange last-modification time (see `evidence/ecosoft_ost/SCHEMA_MAP.md`) |
