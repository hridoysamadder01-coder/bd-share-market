...
| 2026-09-08 | **0** | **spine hash CHANGED — recorded, not hidden** | `c516aed2…` → **`60f41374…`**. Adding the third EcoSoft probe added one claim (2,562 → 2,563) and CI's determinism test caught it immediately. Everything that matters is **unchanged**: 1,078 listings, 692 companies, **657/724 DSE resolved (90.75 %)**, 3 unresolved, gate **7/7 PASS**, rebuild byte-identical. The spine is a function of the evidence, so evidence added means hash changed — the test did exactly its job and the map was regenerated rather than the test weakened |
| 2026-09-08 | **0** | **two defects the CI failure exposed** | **(a)** `load_ecosoft` globbed every probe file but stamped them all with a hard-coded `as_of="2026-09-07"`, so a 2026-09-08 recording carried a false observation date — the one thing an `as_of` exists to carry. Each probe is now dated from its own filename. **(b)** `unresolved` listed one entry per FILE mentioning a code, not per instrument: AAMRANET appeared twice identically and the count read 4. It is now deduplicated by (key, reason) with sources merged, so the unresolved count tracks the problem rather than the size of the evidence directory. Back to 3 |
...
| 2026-09-08 | **1** | **whole market pulled, first time** | 691 DSE listings — the full Stage 0 spine, not a hand-picked list — against every source that publishes with the market shut. **2,043 records, 300 MB raw, `verify_store` all_ok + chain_ok across 22 segments spanning 2 epochs.** Coverage is not uniform and the report says so per source: **395** traded that day (`dsebd_latest`, 384 with volume > 0), **635** carry circuit limits, **473** on StockNow, **472** have fundamentals, **419** a shareholding declaration (1,220 rows over 47 as-on dates — 3 per symbol, the scarce panel §3 named), **387** on CSE. The ~230 treasury bills and bonds inside the 691 are why the rest are "missing": 254 `not_found` on BullBD and 272 `parse_error` on ownership are pages with no such block, recorded as **distinct reasons** because they mean different things. `evidence/public_engine/2026-09-08-full/` |
| 2026-09-08 | **1** | **NOT captured, and why** | No depth, tape, watch, market or block. Those sources are `TRADING_PHASES` only and the run was 15:44 Dhaka, after the 14:10 close. **A closed market publishes the day's outcome and its reference data, never its order flow.** This run is the reference + outcome layer; the whole-market intraday layer still needs a live session |
| 2026-09-08 | **1** | **defect — the engine polled nothing for six minutes** | `time.monotonic()` counts from BOOT on Linux, so the `0.0` the engine used for "never polled" is a real reading, not a sentinel. On a container with 1,359 s of uptime every closed-market cadence (≥ 3600 s) failed `now - 0.0 >= cadence`, and the loop wrote only heartbeats: **17 sources UNTRIED, client requests = 0.** `--once` hid it completely — `run_once` polls without consulting `due` at all, so the manual probe used to check the registry passed while the cadence loop was stalled. Fixed with `NEVER_POLLED = -inf`; `tests/test_public_engine.py` pins it end to end on a patched 1,359 s clock (verified: zero adapter calls with `0.0`) |
| 2026-09-08 | **1** | **defect — a 404 was driving the backoff** | Sweeping 691 listings against BullBD — which lists equities, not T-bills — produced hundreds of consecutive 404s, **each escalating the exponential backoff to its 120 s cap**: 12 misses cost 726 s of waiting, and the live run fell from 100 records in 90 s to 4 requests in 2 min. Backoff is a promise to a *struggling* server, and 404/410 say nothing about load. They are now **neutral** — neither escalating a backoff nor cancelling one a 429 earned — while 429, 403 and 5xx still escalate, and every miss is still counted and still written as a gap |
| 2026-09-08 | **1** | **defect — list-order polling starves the registry** | The scheduler took the first *due* item in registry order. A 691-symbol sweep on a 30 s cadence can never finish inside its own cadence, so one of its symbols is **always** due. Simulated on the real 0.4 s gap over 30 minutes: **depth 1500 polls, tape 0, market 0.** Invisible at 14 symbols, total at market width — and it would have hit tomorrow's open session, not this closed one. `next_due()` now picks the most overdue work **measured in cadences**, so past capacity the whole registry stretches together instead of one source running on time and the rest not at all |
| 2026-09-08 | **1** | **defect — capture breadth that replay could not read** | `replay()` carried its own hand-written routing map covering only the two intraday hosts. **Seven registered sources parsed into nothing, silently** — no error, no empty table, just absence — so this sweep's 691 fundamentals pages and 691 ownership pages were bytes nothing could return. The map now covers the registry, and `tests/test_replay_coverage.py` walks the shipped registry so a source added to the engine fails until replay knows where to put it |
| 2026-09-08 | **1** | **new source, found from a broker terminal's ribbon** | A screenshot of the OST header read `DSEX: 5539.32 \| Tr: 168,534 \| Vol: 191,268,628`. Checking those against the public site turned up `recent_market_information.php` carrying **exactly** them — free, no account. Two figures on it exist in **no other wired source**: **total market capitalisation** (6,911,303.853 mn), the denominator for turnover-to-cap and the only size-free way to compare sessions; and **DSES + DS30 beside DSEX**, without which a broad-index move cannot be separated from one the shariah and blue-chip indices share. Window rolls at ~30 sessions with no archive behind it — same shape as the ownership gap, same remedy: poll from now on. Captured 2026-07-27 → 09-08, `all_ok` + `chain_ok` |
| 2026-09-08 | **1** | **636 company profiles from bytes already on disk** | The same terminal showed `Record Date` and `Dividend` columns; `corporate_event` from BullBD is **0 of 718**, so those looked absent. They were not — `dse_ownership` fetches `displayCompany.php` once per symbol for the shareholding block and **the rest of the page was captured and never read**. A replay-side parser (no fetch, so **zero new requests and retroactive to stores already on disk**) yields: 636 scrip codes, 419 with listing year / category / year-end / short- and long-term loans / reserves, 309 issuer disclosure URLs, and a **cash-dividend record for 371 symbols — 2,984 (symbol, year, percent) rows, running 38 years deep for APEXTANRY**. Raw dividend string kept verbatim beside the parsed pairs: when they later disagree the raw text settles it |
| 2026-09-08 | **1** | **deliberately NOT extracted** | The same page prints the company secretary's **name, personal mobile and personal e-mail**, plus factory address, phone and fax. A research store has no use for a named individual's contact details, so no field carries them and `tests/test_company_profile.py::test_personal_contact_details_are_never_extracted` fails if one ever does. Only the two disclosure URLs — financial statements, price-sensitive information — are kept from that half of the page, because those are what an event study needs |
| 2026-09-08 | **1** | **STILL MISSING** | **Per-symbol issuer news.** The terminal's News panel carries dated announcements — BB-appointed administrator at FAREASTFIN, TB5Y0928 trading suspension, RUNNERAUTO EGM record date 13.09.2026, five funds' NAV declarations. `bsec_publications` is the **regulator's** directives, a different thing from the **issuer's** announcements. DSE's `news_archive.php` exists ("Extended News Search") but its parameters are not documented; guessing them was stopped rather than brute-forced. **One HAR of the news panel resolves it**, the same way the EcoSoft timezone question was resolved |
| 2026-09-08 | **1** | EXISTS | `seeing/extract.py`: store → CSV tables + `COVERAGE.json`. **300 MB of raw HTML becomes 1.9 MB of tables**; the raw store stays local (`.gitignore`) while `MANIFEST.json` keeps every record's sha256 and the per-source hash chain, so any CSV row traces back to the exact bytes it came from. Coverage answers the only question worth asking of a breadth pull — per source, how many of the universe came back and why the rest did not — and a source that returned **nothing at all still gets a line** rather than vanishing |
