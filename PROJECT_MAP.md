...
| D-17 | 2026-09-08 | Lock the build sequence in `ROADMAP.md` — 12 stages plus 4 connective items, documentation-only | the next year of work needed one ordered list with a gate per stage, so a stage cannot be started on an unvalidated dependency and "done" is machine-checkable | `ROADMAP.md`. Four items added as required connectors, not scope: **Reference & Identity Spine** (the 8 sources disagree on the join key, so "the same symbol" was an unstated assumption), **Regime & Panel Separation** (`RESEARCH_STATUS.md` documents 381→88 symbols on 2024-02-22 and the floor era; nothing *enforced* it, so the first sector aggregate would have spanned a basis change), **Cost & Liquidity Model** (gate 3 of `SURVIVING_RESEARCH_LEADS.md`; without it stage 11 cannot judge a policy), **Point-in-Time Discipline** (the repo has been bitten twice — `fwd_mae` as an input, and `flag_stale_run`, which looks ahead by construction at `bdlib/qa.py:40`). Stage 7 is explicitly barred from re-asking the daily-bar question P45-8 already rejected. No code, mechanism, threshold, experiment, adapter or frozen artefact changed |
