...
|---|---|---|
| `tower/normalize.py` | replayed raw tables / parsed frames → `Event`s; seq_local; session phase; t_exch; freshness; duplicate (identical payload per source+symbol), stale (age > cadence×3), gap (heartbeat/GAP records, feed-sequence holes), out-of-order flags; sorted event stream | `normalize_store(root, symbols=None, t_from=None, t_to=None) -> (list[Event], QAStats)`, `events_from_frames(source, frames, ...)`, `QAStats` |
