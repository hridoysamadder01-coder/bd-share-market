...
Time rule (causal): ``?at=<iso>`` answers with the last state whose frame time
``t`` is <= ``at`` (bisect over the per-symbol index, which is in event order);
nothing after ``at`` is ever read to answer a request at ``at``.
...
def _parse_at(at: Optional[str]) -> Optional[datetime]:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"bad time {at!r}: {e}")


def _extract_t(line: bytes) -> datetime:
    ...
    falls back to a full parse when the fast path fails."""
...
class _StateFile:
    ...
    def refresh(self) -> None:
        ...
        if size < self.offset:                # truncated / rewritten: re-index from scratch
            self.offset, self.times, self.spans = 0, [], []
            self._cache.clear()
        ...
        while True:
            ...
            if line.strip():
                try:
                    ...
                    pos = nl + 1
                    continue                   # unreadable line: skipped, never invented
                self.times.append(t)
    ...
    def index_at_or_before(self, at: Optional[datetime]) -> Optional[int]:
        """Index of the last state with t <= at (None → last state; None if none)."""
        if not self.times:
            return None
        if at is None:
    ...
    def range_indices(self, t_from: Optional[datetime], t_to: Optional[datetime]) -> Tuple[int, int]:
        lo = bisect.bisect_left(self.times, t_from) if t_from is not None else 0
        hi = bisect.bisect_right(self.times, t_to) if t_to is not None else len(self.times)
        return lo, hi
...
class StoreReader:
    ...
    # ------------------------------------------------------------ queries
    def state_at(self, symbol: str, at: Optional[datetime]) -> Dict[str, Any]:
        f = self.file(symbol)
        i = f.index_at_or_before(at)
        if i is None:
            raise HTTPException(status_code=404, detail=f"no state of {symbol} at or before {at.isoformat()}")
        d = f.read(i)
        return {
            "symbol": symbol, "index": i, "count": len(f), "at": at.isoformat() if at else "latest",
    ...
    def episodes(self, symbol: str, t_from: Optional[datetime], t_to: Optional[datetime]) -> List[Dict[str, Any]]:
        ...
        for r in self.timeline.rows:
            ...
            if ep is None and to in EPISODE_OPEN:
                open_[name] = {"name": name, "start": r["t"], "end": None, "peak_state": to, "outcome": None}
            elif ep is not None:
    ...
    def cross(self, symbol: str, at: Optional[datetime]) -> Dict[str, Any]:
        """Cross-stock view as of ``at``: the symbol's own cross/sector context plus the
        state (at or before ``at``) of every related symbol — leaders, laggers and
        same-sector members present in the store."""
        s = self.state_at(symbol, at)
        st = s["state"]
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "states": {s: {"count": len(f), "bytes": f.offset, "last_t": f.times[-1].isoformat()}
                       for s, f in self.files.items() if len(f)},
...
def create_app(store: str) -> FastAPI:
    ...
    def api_state(symbol: str, at: Optional[str] = Query(default="latest")) -> Any:
        reader.refresh()
        return reader.state_at(symbol, _parse_at(at))
    ...
    def api_history(symbol: str, fields: str = Query(default="mid,ltp"), from_: Optional[str] = Query(default=None, alias="from"),
        ...
        return reader.history(symbol, flds, _parse_at(from_), _parse_at(to), max_points)
    ...
    def api_timeline_symbol(symbol: str, from_: Optional[str] = Query(default=None, alias="from"),
        ...
        reader.refresh()
        reader.file(symbol)
        rows = reader.timeline.select(symbol, _parse_at(from_), _parse_at(to))
    ...
    def api_timeline(from_: Optional[str] = Query(default=None, alias="from"), to: Optional[str] = Query(default=None)) -> Any:
        reader.refresh()
        rows = reader.timeline.select(None, _parse_at(from_), _parse_at(to))
    ...
    def api_cross(symbol: str, at: Optional[str] = Query(default="latest")) -> Any:
        reader.refresh()
        return reader.cross(symbol, _parse_at(at))
    ...
    def api_replay_seek(symbol: str, at: str) -> Any:
        reader.refresh()
        s = reader.state_at(symbol, _parse_at(at))
